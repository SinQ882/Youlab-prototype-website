...
export default function HeroSection({ navigate }) {
  return (
    <>
      ...
      {/* ── Stats + Logo strip ───────────────────── */}
      <section className="bg-muted/50 border-y border-border py-7">
        <div className="max-w-[1120px] mx-auto px-6">
          <div className="flex flex-wrap gap-6 items-center justify-between">
            ...
            <div className="flex flex-wrap gap-x-10 gap-y-2">
              {stats.map((s, i) => (
                <div key={i} className="text-center">
                  <div className="text-2xl font-black text-foreground tracking-tight leading-none">{s.value}</div>
            ...
            {/* Partner logos */}
            <div className="flex items-center flex-wrap gap-x-6 gap-y-1.5 opacity-40">
              <span className="text-[11px] font-bold text-foreground uppercase tracking-widest">
                Vertrouwd door
              </span>
              {logos.map((l, i) => (
                <span key={i} className="text-[15px] font-extrabold text-foreground tracking-tight">{l}</span>
              ))}
            </div>
          </div>
        </div>
